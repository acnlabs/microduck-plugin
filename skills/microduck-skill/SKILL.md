...
3. **Package** — `$SKILL/scripts/export_publish.sh`. [references/PUBLISH.md](references/PUBLISH.md). After a tensorboard run, export from `--checkpoint-file` (or an already-gated `--onnx`), not a wandb path. Then try `$SKILL/scripts/preview.sh --repo <this-run-repo> --task …` (or `--mp4` if a clip already exists). That is a sim checkpoint replay on **this** Hub card (`preview.mp4`), not ONNX control, not a robot, not a ranking. `play.sh` is a viewer and does not exit; preview starts it, waits for the clip, then stops it. Play/upload failure is not a failed publish — say `preview skipped` and continue. Do not put mp4 in the plugin git.
